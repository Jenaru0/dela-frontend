...
import { X, Save, AlertTriangle, Clock, CheckCircle, XCircle, MessageCircle } from 'lucide-react';
...
const ReclamoManageModal: React.FC<ReclamoManageModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.prioridad !== reclamo.prioridad) changes.prioridad = formData.prioridad;
      if (formData.tipoReclamo !== reclamo.tipoReclamo) changes.tipoReclamo = formData.tipoReclamo;
      if (formData.asunto !== reclamo.asunto) changes.asunto = formData.asunto;
      if (formData.descripcion !== reclamo.descripcion) changes.descripcion = formData.descripcion;
  ...
  const getStatusIcon = (estado: EstadoReclamo) => {
    switch (estado) {
      ...
      case EstadoReclamo.EN_PROCESO:
        return <MessageCircle className="w-4 h-4" />;      case EstadoReclamo.RESUELTO:
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-[#9A8C61] text-sm">ID: #{reclamo.id} - {reclamo.asunto}</p>
            </div>
          </div>
          <Button
            variant="ghost"
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, estado: e.target.value as EstadoReclamo })}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, prioridad: e.target.value as PrioridadReclamo })}
                  className="w-full px-4 py-2 border border-[#ecd8ab]/50 rounded-lg focus:border-[#CC9F53] focus:ring-[#CC9F53]/20"
                >
                ...
                {formData.prioridad && (
                  <div className="mt-2 flex items-center text-sm text-[#9A8C61]">
                    ...
                    <span className={getPriorityIcon(formData.prioridad) ? "ml-1" : ""}>
                      Prioridad actual
                    </span>
                  </div>
                )}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setFormData({ ...formData, tipoReclamo: e.target.value as TipoReclamo })}
                className="w-full px-4 py-2 border border-[#ecd8ab]/50 rounded-lg focus:border-[#CC9F53] focus:ring-[#CC9F53]/20"
              >
            ...
            <div>
              ...
              <p className="text-xs text-[#9A8C61] mt-1">
                Esta nota se agregará como comentario interno si se proporcionan cambios.
              </p>
            </div>
            ...
            {hasChanges() && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <ul className="space-y-1 text-sm text-blue-800">
                  {formData.estado !== reclamo.estado && (
                    <li>• Estado: {EstadoReclamoLabels[reclamo.estado]} → {EstadoReclamoLabels[formData.estado!]}</li>
                  )}
                  {formData.prioridad !== reclamo.prioridad && (
                    <li>• Prioridad: {PrioridadReclamoLabels[reclamo.prioridad]} → {PrioridadReclamoLabels[formData.prioridad!]}</li>
                  )}
                  {formData.tipoReclamo !== reclamo.tipoReclamo && (
                    <li>• Tipo: {TipoReclamoLabels[reclamo.tipoReclamo]} → {TipoReclamoLabels[formData.tipoReclamo!]}</li>
                  )}
                  {formData.asunto !== reclamo.asunto && (
