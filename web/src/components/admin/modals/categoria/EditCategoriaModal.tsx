...
import { categoriasService, UpdateCategoriaDto, Categoria } from '@/services/categorias.service';
...
const EditCategoriaModal: React.FC<EditCategoriaModalProps> = ({
  ...
  const [formData, setFormData] = useState<UpdateCategoriaDto>({
    nombre: '',
    descripcion: '',
    slug: '',
    activo: true,
  ...
  const handleInputChange = (field: keyof UpdateCategoriaDto, value: string | boolean) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Error desconocido al actualizar categoría';
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-neutral-200">
          <h2 className="text-xl font-bold text-neutral-900">Editar Categoría</h2>
          <button
            onClick={onClose}
